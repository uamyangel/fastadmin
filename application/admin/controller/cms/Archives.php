...
class Archives extends Backend
{
    ...
    public function get_channel_fields()
    {
        ...
        if ($channel && $channel['type'] === 'list') {
            ...
            //检查模型是否有标签字段
            $tagFields = \app\admin\model\tags\Category::where('deleted_at is null and find_in_set(:canuse_model_ids, canuse_model_ids)', ['canuse_model_ids' => $channel['model_id']])
                ->order('id asc')
                ->select();
            if($tagFields){
                //临时字段，用于替换为标签字段
                $tmpFields = \app\admin\model\cms\Fields::where('1=1')
                    ->order('id desc')
                    ->limit(count($tagFields))
                    ->select();

                foreach($tagFields as $key => $item){
                    $last = $tmpFields[$key];
                    $last->model_id = $channel['model_id'];
                    $last->diyform_id = 0;
                    $last->name = 'module_tags';
                    $last->type = 'checkbox';
                    $last->title = $item->name;
                    $last->content = '#SQL#select id,name from fa_tags where category_id='.$item->id.' and deleted_at is null';
                    $last->defaultvalue = '';
                    $last->rule = '';
                    $last->msg = '';
                    $last->ok = '';
